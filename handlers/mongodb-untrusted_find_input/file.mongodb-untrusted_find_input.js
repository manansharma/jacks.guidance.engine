...
function MongoUntrustedFindInputFileWriter(blackboard) {
    this.currentState = 0;
    this.name = 'MongoUntrustedFindInputFileWriter';
    ...
    this.ready = function() {
        return blackboard.count({trigger: MongoUntrustedFindInput.ID, type: Constants.Type.EVENT}) > 0;
    };

    this.process = function(callback) {
        _self.currentState = 1;
        blackboard.take({
            template: {trigger: MongoUntrustedFindInput.ID, type: Constants.Type.EVENT},
            ontake: function(event) {

                logger.debug("Adding new file to Best Practice: " + event.filename);
                var file = new File(MongoUntrustedFindInput.ID, event.filename, 'Javascript');
...
function create(blackboard) {
    return new MongoUntrustedFindInputFileWriter(blackboard);
}
