...
var HttpOnlySession = require('./express-httponlysession');
...
function ExpressHttpOnlySessionAdviceAssembler(blackboard) {
  this.currentState = 0;
  this.name = 'ExpressHttpOnlySessionAdviceAssembler';
  ...
  this.ready = function() {
    return blackboard.count({trigger: HttpOnlySession.ID, type: Constants.Type.EVENT}) > 0;
  };

  this.process = function(callback) {
    _self.currentState = 1;
    blackboard.take({
      template: {trigger: HttpOnlySession.ID, type: Constants.Type.EVENT},
      ontake: function(event) {
        ...
        var file_evidence_template = HttpOnlySession.EVIDENCES[evidence_type].template;
        var guidanceTitle = HttpOnlySession.EVIDENCES[evidence_type].title;
        ...
        fs.readFile(event.filename, 'utf8', function(err, data) {
          ...
          if(Array.isArray(event.patch)) {
            event.patch.forEach(function(patch) {
              if(patch.action === Constants.PatchActions.UPDATE) {
                ...
                if(evidence_type === "set-httponly-to-true-in-a-separate-call") {
          ...
          var advice = new Advice(HttpOnlySession.ID, Constants.Modality.PATCH, evidence_rendered);

          var patchfile = new File(HttpOnlySession.ID, event.filename, 'Javascript', data, lines.join('\n'));
          patchfile.event_id = event.id;
          advice.files = [patchfile];

          var guidance = new Guidance(event_id, HttpOnlySession.ID, event.category, event, guidanceTitle);
...
function create(blackboard) {
  return new ExpressHttpOnlySessionAdviceAssembler(blackboard);
}
