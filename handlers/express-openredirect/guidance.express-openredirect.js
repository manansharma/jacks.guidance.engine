...
var OpenRedirect = require('./express-openredirect');
...
function GuidanceBestPracticeOpenRedirect(blackboard) {
  this.currentState = 0;
  this.name = 'GuidanceBestPracticeOpenRedirect';
  ...
  this.ready = function() {
    return blackboard.count({trigger: OpenRedirect.ID, type: Constants.Type.EVENT}) > 0;
  };

  this.process = function(callback) {
    _self.currentState = 1;
    blackboard.take({
      template: {trigger: OpenRedirect.ID, type: Constants.Type.EVENT},
      ontake: function(event) {
        ...
        var guidanceTitle = OpenRedirect.BP_TITLE;
        ...
        try {
          ...
          var evidence_model = {
            ...
            untrusted_redirect_code: stack[stack.length-1].code.trim(),
            trace_route: JSON.stringify(route),
            trace_stack: JSON.stringify(stack, null, 2)
          };
        } catch(e) {
          logger.error(new Error('Failure in retrieving code of untrusted open redirect: ' + e.message));
        ...
        var advice = new Advice(OpenRedirect.ID, Constants.Modality.BEST_PRACTICE, evidence_rendered);

        logger.debug("Adding new file to Best Practice: " + event.filename);
        var file = new File(OpenRedirect.ID, event.filename, 'HTML');
        ...
        var guidance = new Guidance(event_id, OpenRedirect.ID, Constants.Category.ROUTE, event, guidanceTitle);
...
function create(blackboard) {
  return new GuidanceBestPracticeOpenRedirect(blackboard);
}
