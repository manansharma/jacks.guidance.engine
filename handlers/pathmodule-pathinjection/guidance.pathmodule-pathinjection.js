...
var PathInjection = require('./pathmodule-pathinjection');
...
function GuidancePathInjection(blackboard) {
  this.currentState = 0;
  this.name = 'GuidancePathInjection';
  ...
  this.ready = function() {
    return blackboard.count({trigger: PathInjection.ID, type: Constants.Type.EVENT}) > 0;
  };

  this.process = function(callback) {
    _self.currentState = 1;
    blackboard.take({
      template: {trigger: PathInjection.ID, type: Constants.Type.EVENT},
      ontake: function(event) {
        ...
        var guidanceTitle = PathInjection.BP_TITLE;
        ...
        try {
          ...
          var evidence_model = {
            line_number: innerStack[innerStack.length-1].line,
            source_file: event.stack.src,
            untrusted_pathInjection_code: innerStack[innerStack.length-1].code.trim(),
        ...
        var advice = new Advice(PathInjection.ID, Constants.Modality.BEST_PRACTICE, evidence_rendered);

        logger.debug("Adding new file to Best Practice: " + event.filename);
        var file = new File(PathInjection.ID, event.filename, 'HTML');
        ...
        var guidance = new Guidance(event.id, PathInjection.ID, Constants.Category.ROUTE, event, guidanceTitle);
...
function create(blackboard) {
  return new GuidancePathInjection(blackboard);
}
